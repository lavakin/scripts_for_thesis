...
dataset = {'molecule':[], 'boiling point':[],'wiener':[],'sum_connectivity':[],'randic':[],'balaban':[], 'szeged':[], 'revised szeged':[],'detour':[],'harary':[], 'path_walk_ratio 1.1':[],'path_walk_ratio 1.2':[],'path_walk_ratio 2.1':[],'path_walk_ratio 2.2':[], 'estrada 2':[],'estrada 3':[],'markov 2':[],'markov 3':[],'wiener_hosoya':[]}
...
for index, row in chemicals.iterrows(): 
    ...
    if atoms.issubset({'C','H'}) and len(G)<26:
        try:
        ...
        dataset['estrada 3'].append(Walk.estrada(G,3))
        dataset['markov 2'].append(Walk.markov(G,2))
        dataset['markov 3'].append(Walk.markov(G,3))
        i+=1
        if i%20:
            print(i)
...
print(original_df)
original_df.to_pickle("all.pickle")
